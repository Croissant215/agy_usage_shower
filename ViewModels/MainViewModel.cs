    ...
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        public UsageData CurrentUsage
        {
            ...
            set
            {
                _currentUsage = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(QuotaColorHex));
                OnPropertyChanged(nameof(QuotaFormatted));
            }
        }

        public string QuotaFormatted => $"{CurrentUsage.QuotaPercent}%";

        public string QuotaColorHex
        {
            get
            {
                if (CurrentUsage.QuotaPercent >= 70) return "#40C057"; // Green
                if (CurrentUsage.QuotaPercent >= 30) return "#FAB005"; // Yellow
                return "#FA5252"; // Red
            }
        }
